...
BEGIN
    ...
    VALUES (v_user_key, 0, v_now - interval '1 hour', v_now)
    ...
    IF v_user_row.last_window_reset_20s IS NULL OR v_now - v_user_row.last_window_reset_20s >= interval '20 seconds' THEN
        v_user_row.count_20s := 0;
        v_user_row.last_window_reset_20s := v_now;
    END IF;
    IF v_user_row.last_window_reset_10m IS NULL OR v_now - v_user_row.last_window_reset_10m >= interval '10 minutes' THEN
        v_user_row.count_10m := 0;
        v_user_row.last_window_reset_10m := v_now;
    END IF;
    IF v_user_row.last_window_reset_1h IS NULL OR v_now - v_user_row.last_window_reset_1h >= interval '1 hour' THEN
        v_user_row.count_1h := 0;
        v_user_row.last_window_reset_1h := v_now;
    END IF;

    IF COALESCE(v_user_row.count_20s, 0) >= v_limit_20s THEN
    ...
    END IF;
    IF COALESCE(v_user_row.count_10m, 0) >= v_limit_10m THEN
        RETURN jsonb_build_object('success', false, 'reason', 'rate_limit', 'message', 'Limite de diagnósticos por 10 minutos atingido.');
    END IF;
    IF COALESCE(v_user_row.count_1h, 0) >= v_limit_1h THEN
        RETURN jsonb_build_object('success', false, 'reason', 'rate_limit', 'message', 'Limite de diagnósticos por hora atingido.');
    END IF;

    v_effective_server_limit := public.get_server_concurrency_limit(p_server_id, p_max_server_concurrent);

    INSERT INTO public.diagnostic_concurrency_control (key, active_count, last_request_at, updated_at)
    VALUES (v_server_key, 0, v_now - interval '1 hour', v_now)
    ON CONFLICT (key) DO NOTHING;
    ...
    BEGIN
        ...
        IF v_last_srv_req IS NOT NULL AND v_now - v_last_srv_req < interval '5 seconds' THEN
             RETURN jsonb_build_object('success', false, 'reason', 'server_cooldown', 'message', 'Aguarde 5 segundos entre testes no mesmo servidor.');
        END IF;
    END;

    -- IMPORTANTE: usar os contadores já normalizados (janela reiniciada), não os valores antigos da tabela
    UPDATE public.diagnostic_concurrency_control SET
        active_count = active_count + 1,
        count_20s = COALESCE(v_user_row.count_20s, 0) + 1,
        count_10m = COALESCE(v_user_row.count_10m, 0) + 1,
        count_1h = COALESCE(v_user_row.count_1h, 0) + 1,
