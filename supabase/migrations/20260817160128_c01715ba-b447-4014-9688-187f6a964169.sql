CREATE OR REPLACE FUNCTION public.get_admin_resellers_v2()
 RETURNS TABLE(id uuid, email text, full_name text, created_at timestamp with time zone, credits integer, parent_id uuid, owner_id uuid, sub_reseller_count bigint, client_count bigint, last_activity_at timestamp with time zone)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT
    ...
    (SELECT max(ch.created_at) FROM public.credit_history ch WHERE ch.user_id = p.id) AS last_activity_at
