...
export type AlertCandidate = {
  ...
  title: string;
  detail?: string | null;
  /** Quantas detecções seguidas são necessárias antes de avisar (padrão 1). */
  confirmations?: number;
  /** Eventos pontuais (catálogo, mudança de IP): sempre notificam, sem estado. */
  transient?: boolean;
};
...
export async function dispatchAlerts(
  ...
  const byKind = new Map<string, StateRow>((rows ?? []).map((r) => [r.kind, r as StateRow]));

  const toNotify: AlertCandidate[] = [...transient];
  ...
  for (const c of stateful) {
    ...
    if (!alreadyActive && confirmed) toNotify.push(c);
  }
  ...
  if (!toNotify.length) return { notified: 0, resolved: resolvedKinds.length };
  ...
  try {
    ...
    await notifyServerIptvAlerts(
      serverId,
      toNotify.map((c) => ({ title: c.title, detail: c.detail ?? "", severity: c.severity })),
    );
  } catch {
