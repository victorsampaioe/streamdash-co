...
export type IptvCredentials = { username: string | null; password: string | null };
 ...
 */
export async function getIptvCredentials(serverId: string): Promise<IptvCredentials> {
  const { data } = await supabaseAdmin
    .from("servers")
    .select("iptv_username, iptv_password")
    .eq("id", serverId)
    .maybeSingle();
  if (!data) return { username: null, password: null };
  ...
  const needsMigration =
    (data.iptv_username && !isEncrypted(data.iptv_username)) ||
...
export async function setIptvCredentials(serverId: string, creds: IptvCredentials) {
...
export async function migratePlaintextCredentials(limit = 200) {
  ...
  let migrated = 0;
  for (const s of data ?? []) {
    if (isEncrypted(s.iptv_username ?? "") && isEncrypted(s.iptv_password ?? "")) continue;
    if (!s.iptv_username && !s.iptv_password) continue;
    await supabaseAdmin
      .from("servers")
      .update({
...
function blockMinutes(failures: number): number | null {
  if (failures < MAX_FAILURES) return null;
  if (failures < 7) return 15;
  if (failures < 10) return 60;
  return 360;
}

export type LoginGuard = { allowed: boolean; blockedUntil: string | null; failures: number };

export async function checkLoginGuard(serverId: string): Promise<LoginGuard> {
  const { data } = await supabaseAdmin
...
export function guardMessage(guard: LoginGuard): string {
...
export async function registerLoginResult(
  ...
  if (ok) {
    await supabaseAdmin.from("iptv_login_attempts").upsert(
      { server_id: serverId, failures: 0, last_attempt_at: nowIso, blocked_until: null, last_reason: null },
  ...
  const blockedUntil = mins ? new Date(Date.now() + mins * 60_000).toISOString() : null;
...
export async function resetLoginAttempts(serverId: string) {
  await supabaseAdmin.from("iptv_login_attempts").upsert(
    { server_id: serverId, failures: 0, blocked_until: null, last_reason: null },
    { onConflict: "server_id" },
  );
}
