...
export const savePlayerSettings = createServerFn({ method: "POST" })
    ...
    const userClaims = context.claims;
...
export const loginXtreamClient = createServerFn({ method: "POST" })
    ...
    let authResult;
    try {
        authResult = await runOnCore("iptv-validate", {
            host: server.host,
    ...
    if (!authResult?.login_ok) {
        ...
        if (errorMsg.includes("403"))
            throw new Error("Acesso temporariamente bloqueado pelo servidor (403).");
        ...
        if (errorMsg.includes("inválidos"))
...
export const getPlayerCatalog = createServerFn({ method: "POST" })
    ...
    const creds = await getPlayerCredentials(session);
    ...
    const via = useCore("iptv-player-proxy") ? `Core AWS (${coreApiUrl()})` : "Painel (direto no IPTV, dono do banco)";
    const started = Date.now();
    console.log(`[CATALOG_DEBUG] inicio | tipo=${data.action.includes("series") ? "series" : data.action.includes("vod") ? "movie" : "live"} servidor=${session.server_id} usuario_iptv=${creds.username} action=${data.action} categoria=${data.categoryId ?? "-"} contentId=${data.contentId ?? "-"} fluxo=Frontend -> ${via} -> Servidor IPTV`);
    try {
        ...
        const quantidade = Array.isArray(result)
            ...
            : result && typeof result === "object"
                ...
                : 0;
        console.log(`[CATALOG_DEBUG] resposta | action=${data.action} via=${via} ms=${Date.now() - started} tipo_resposta=${Array.isArray(result) ? "array" : typeof result} quantidade=${quantidade} erro=null`);
        return result;
    }
    catch (e) {
        console.error(`[CATALOG_DEBUG] resposta | action=${data.action} via=${via} ms=${Date.now() - started} quantidade=0 erro=${e?.message ?? e}`);
...
export const diagnosePlayerCatalog = createServerFn({ method: "POST" })
    ...
    const creds = await getPlayerCredentials(session);
    if (!creds.username || !creds.password)
        throw new Error("Credenciais da sessão indisponíveis");
    ...
    let coreSaude = { alcancavel: false };
    const coreBase = coreApiUrl();
    if (coreBase) {
        try {
            const res = await fetch(`${coreBase}/api/public/core/task`, { signal: AbortSignal.timeout(10000) });
    ...
    return {
        core: {
            ...
            usando_core: useCore("iptv-player-proxy"),
            fluxo: useCore("iptv-player-proxy") ? "Frontend -> Core AWS -> Servidor IPTV" : "Frontend -> Painel -> Servidor IPTV",
...
export const getTMDBMetadata = createServerFn({ method: "GET" })
    ...
    try {
        ...
        if (data.year)
            searchUrl.searchParams.append('year', data.year);
        ...
        return {
            ...
            cast: detailsData.credits?.cast?.slice(0, 5).map((c) => c.name) || [],
            trailer_key: detailsData.videos?.results?.find((v) => v.type === 'Trailer' && v.site === 'YouTube')?.key
        };
    }
    catch (error) {
...
export const updatePlayerActivity = createServerFn({ method: "POST" })
    ...
    if (data.progress !== undefined)
        activityData.progress = data.progress;
    if (data.isFavorite !== undefined)
        activityData.is_favorite = data.isFavorite;
    if (data.metadata)
...
export const getPlayerActivity = createServerFn({ method: "GET" })
    ...
    if (data.type === 'progress')
        query = query.gt("progress", 0).lt("progress", 95);
