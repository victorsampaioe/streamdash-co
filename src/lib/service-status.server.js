 ...
 * Admins always pass.
 */
export async function isServiceActive(userId) {
 ...
 */
export async function getActiveOwnerIds(ownerIds) {
    ...
    const walletMap = new Map((wallets ?? []).map((w) => [w.reseller_id, w.credits ?? 0]));
...
export async function isServerMonitoringAllowed(serverId) {
...
export const MONITORING_PAUSED_MESSAGE = "Monitoramento pausado: assinatura expirada ou sem créditos. Renove para reativar as verificações.";
 ...
 */
export async function syncServersPauseState(ownerIds) {
    let query = supabaseAdmin.from("servers").select("id, owner_id, monitoring_paused, paused_reason");
    if (ownerIds?.length)
        query = query.in("owner_id", Array.from(new Set(ownerIds.filter(Boolean))));
    ...
    const toPause = servers
        .filter((s) => !active.has(s.owner_id) && !s.monitoring_paused)
        .map((s) => s.id);
    const toResume = servers
        .filter((s) => active.has(s.owner_id) && s.monitoring_paused && s.paused_reason === PAUSE_REASON_EXPIRED)
        .map((s) => s.id);
    if (toPause.length) {
        await supabaseAdmin
            .from("servers")
