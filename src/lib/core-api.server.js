 ...
 */
export const WORKER_CAPABLE_TASKS = new Set([
    "probe-http",
    "probe-dns",
    "probe-iptv-login",
    "iptv-detect",
...
export function canRunOnCore(task) {
    return WORKER_CAPABLE_TASKS.has(task);
}
export function useCore(task) {
    if (!coreApiUrl() || isCoreInstance())
        return false;
    if (task && !canRunOnCore(task))
        return false;
    return true;
}
/**
    ...
    try {
        ...
        if (id) {
            ...
            const { data: inserted, error } = await supabaseAdmin
                ...
                .single();
            if (error)
                console.error("[core-audit] Error inserting log:", error);
            return inserted?.id;
        }
    }
    catch (e) {
...
function sanitizePayload(payload) {
    ...
    if ("password" in clone)
        clone["password"] = clone["password"] ? "***presente***" : null;
    return clone;
}
...
export async function callCore(task, payload = {}) {
    ...
    if (!canRunOnCore(task)) {
        throw new Error(`Tarefa "${task}" depende do banco e roda no Painel — não é delegável ao Core worker.`);
    }
    ...
    console.log(`[core-task] ▶ início task=${task} fila=${queueWaitMs}ms emExecucao=${inFlight}/${MAX_CONCURRENT} aguardando=${waiting.length} timeout=${timeoutMs}ms`);
    ...
    console.log(`[CORE REQUEST] task: ${task} | host: ${payload.host || "N/A"} | username: ${payload.username ? "presente" : "ausente"} | password: ${payload.password ? "presente" : "ausente"} | secret: ${secret ? "presente" : "ausente"} | timestamp: ${new Date().toISOString()}`);
    try {
        ...
        const unwrapped = json && typeof json === "object" && "success" in json && "result" in json
            ? json.result
            : json;
        return unwrapped;
    }
        ...
        console.error(`[core-task] ✖ fim task=${task} status=${isTimeout ? "timeout" : "erro"} duracao=${elapsed}ms erro=${e?.message}`);
        throw e;
    }
    finally {
...
export async function runOnCore(task, payload, local) {
    if (!useCore(task)) {
        if (coreApiUrl() && !isCoreInstance() && !canRunOnCore(task)) {
            console.log(`[CORE SKIP] task: ${task} | motivo: tarefa depende do banco (Painel é dono do banco) | timestamp: ${new Date().toISOString()}`);
        }
        return await local();
    }
    try {
 ...
 */
export async function coreJsonPost(path, timeoutMs = 20000) {
    const base = coreApiUrl();
    if (!base)
        throw new Error("CORE_API_URL não configurada");
