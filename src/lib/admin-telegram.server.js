...
export async function notifyAdmin(text) {
    ...
    if (!token || !chatId)
        return { ok: false, error: "TELEGRAM_BOT_TOKEN/ADMIN_TELEGRAM_CHAT_ID ausente" };
    try {
...
async function notifyUserTelegram(userId, text) {
    const token = process.env.TELEGRAM_BOT_TOKEN;
    if (!token)
        return;
...
export async function notifyNewlyExpiredSubscriptions() {
...
export const EXPIRED_ACCESS_MESSAGE = "⚠️ <b>Seu acesso expirou</b>\n\n" +
    ...
    "👉 https://streammonitor.site/app/subscription";
export async function notifyExpiredAccessUsers() {
    ...
    const { data: channels } = await supabaseAdmin
        ...
        .eq("kind", "telegram")
        .eq("enabled", true);
    const candidates = Array.from(new Set((channels ?? []).map((c) => c.owner_id)));
...
export async function broadcastGlobalIncident(message) {
    const token = process.env.TELEGRAM_BOT_TOKEN;
    if (!token)
        return;
...
export async function notifyAdminSignup(data) {
...
export async function getReactivationStats() {
    const { data: lastCampaign } = await supabaseAdmin
        ...
        .limit(1)
        .maybeSingle();
    const { data: expiredSubs } = await supabaseAdmin.from("subscriptions").select("user_id").eq("status", "expired");
    const expiredIds = (expiredSubs || []).map((s) => s.user_id);
...
export async function runReactivationCampaign(manual = false) {
    const token = process.env.TELEGRAM_BOT_TOKEN;
    if (!token)
    ...
    try {
        ...
        if (!expiredSubs?.length) {
            await supabaseAdmin.from("reactivation_campaigns").update({ status: "completed", finished_at: new Date().toISOString() }).eq("id", campaign.id);
        ...
        const { data: channels } = await supabaseAdmin
            ...
            .in("owner_id", userIds);
        if (!channels?.length) {
            await supabaseAdmin.from("reactivation_campaigns").update({ status: "completed", total_found: userIds.length, finished_at: new Date().toISOString() }).eq("id", campaign.id);
        ...
        await supabaseAdmin.from("reactivation_campaigns").update({
            status: "completed",
