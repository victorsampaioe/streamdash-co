...
function Dashboard() {
    ...
    const { data: servers = [], refetch } = useQuery({
        ...
        queryFn: async () => {
            const { data, error } = await supabase
                .from("servers")
                .select("id, name, description, category, current_status, last_latency_ms, last_checked_at, ssl_days_remaining, health_score, dns_health_score, is_public, public_slug, created_at")
    ...
    const { data: alerts = [] } = useQuery({
        ...
        queryFn: async () => {
            ...
            const items = [
                ...(dns.data ?? []).map((a) => ({ id: `dns-${a.id}`, kind: "DNS", title: a.title, detail: a.detail, at: a.created_at, severity: a.severity, server: nameById.get(a.server_id) })),
                ...(iptv.data ?? []).map((a) => ({ id: `iptv-${a.id}`, kind: "IPTV", title: a.title, detail: a.detail, at: a.created_at, severity: a.severity, server: nameById.get(a.server_id) })),
                ...(inc.data ?? []).map((a) => ({
                    id: `inc-${a.id}`,
                    kind: "Servidor",
    ...
    const filtered = useMemo(() => {
        return servers.filter((s) => {
            if (filter !== "all" && s.current_status !== filter)
                return false;
    ...
    const iptvHealth = useMemo(() => {
        const list = servers.filter((s) => s.health_score != null);
        if (!list.length)
            return null;
        return Math.round(list.reduce((sum, s) => sum + s.health_score, 0) / list.length);
    }, [servers]);
    const firstName = (profile?.full_name || profile?.email || "").split(/[\s@]/)[0] || "";
    const { data: sub } = useSubscription();
    return (<div className="space-y-6">
      {(sub?.profile?.is_reseller && sub?.profile?.credits === 0) ? (<Alert variant="destructive" className="bg-destructive/10 border-destructive/20 mb-6">
        ...
        </Alert>) : sub?.isExpired ? (<Alert variant="destructive" className="bg-destructive/10 border-destructive/20 mb-6">
          <AlertCircle className="h-4 w-4"/>
          <AlertDescription className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="p-5 flex flex-col justify-between">
          ...
          <div>
            ...
            {iptvHealth != null && (<div className="mt-4 flex items-center justify-between text-xs">
      ...
      <Card className="p-5">
        ...
        {alerts.length === 0 ? (<p className="text-sm text-muted-foreground">Nenhum alerta recente. Tudo tranquilo por aqui ✨</p>) : (<ul className="divide-y divide-border/60">
            {alerts.map((a) => (<li key={a.id} className="py-2.5 flex items-start gap-3">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex gap-1 rounded-md border border-border/60 bg-card/60 p-1 text-xs">
          {["all", "up", "degraded", "down"].map((f) => (<button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 rounded ${filter === f ? "bg-accent text-accent-foreground" : "text-muted-foreground hover:text-foreground"}`}>
      ...
      {filtered.length === 0 ? (<Card className="p-12 text-center border-dashed">
...
function HealthBar({ value }) {
    ...
    return (<div className="flex gap-1">
      {Array.from({ length: 10 }, (_, i) => (<div key={i} className={`h-2.5 flex-1 rounded-sm ${i < filled ? tone : "bg-muted/50"}`}/>))}
    </div>);
}
function Summary({ emoji, label, value, tone }) {
    return (<div>
...
function ServerCard({ server }) {
    ...
    return (<Link to="/app/servers/$id" params={{ id: server.id }}>
      <Card className="p-5 hover:border-primary/50 transition-colors group">
        ...
        <div className="flex items-center justify-between text-xs mb-3">
          ...
          {server.ssl_days_remaining != null && (<Badge variant="outline" className="font-mono">SSL {server.ssl_days_remaining}d</Badge>)}
        </div>
