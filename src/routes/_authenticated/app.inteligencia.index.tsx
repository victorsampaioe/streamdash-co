    ...
    <div className="space-y-5">
      ...
      <Dialog open={showConfirm} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {syncProgress ? (syncProgress.processed === syncProgress.total ? "Relatório após sincronização" : "Sincronização em Progresso") : "Restringir sincronização do Radar IPTV somente para Admin"}
            </DialogTitle>
            <DialogDescription className="text-xs mt-2 whitespace-pre-wrap">
              {syncProgress ? (
                syncProgress.processed === syncProgress.total ? (
                  "Restringir sincronização do Radar IPTV somente para Admin\n\nA função de sincronização do Radar IPTV deve ser exclusiva do administrador principal.\n\nCorrigir permissões:\n\nADMIN:\n\nvisualizar Radar completo;\nbotão \"Sincronizar conteúdos agora\";\niniciar jobs;\nacompanhar logs;\nver servidores processados.\n\nREVendedores:\n\nremover botão \"Sincronizar conteúdos agora\";\nnão permitir iniciar sincronização;\nnão acessar logs internos;\nnão executar jobs.\n\nClientes:\n\nsem acesso ao Radar administrativo.\n\nValidar a proteção também no backend/API, não somente escondendo o botão no frontend.\n\nCaso um revendedor tente chamar a API diretamente, retornar acesso negado. e Remover definitivamente o Monitor de Conteúdos Offline\n\nQuero desativar e remover a função Monitor de Conteúdos Offline do sistema.\n\nPágina atual:\nhttps://streammonitor.site/app/conteudos\n\nAlterações necessárias:\n\nRemover do menu lateral:\n\"Conteúdos Offline\"\nRemover acesso da rota:\n/app/conteudos\nCaso alguém tente acessar diretamente o link:\nredirecionar para /app/inteligencia\nou retornar página não disponível.\nRemover componentes, chamadas e processos relacionados ao Monitor de Conteúdos Offline para não consumir recursos.\nNão afetar o:\nRadar de Conteúdo;\nInteligência de Conteúdo IPTV;\nRanking;\nSincronização TMDB;\nPrimeiras detecções.\n\nManter somente o novo sistema de Radar Inteligente de Conteúdo como módulo oficial."
                ) : (
