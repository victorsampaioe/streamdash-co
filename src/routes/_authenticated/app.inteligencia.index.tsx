            ...
            <DialogDescription className="text-xs mt-2 whitespace-pre-wrap">
              {syncProgress ? (
                syncProgress.processed === syncProgress.total ? (
                  "Restaurar Ranking de Atualização do Radar sem quebrar dados existentes\n\nApós ajustes na sincronização do Radar IPTV, o Ranking Atualização mudou e perdeu o comportamento anterior.\n\nRestaurar o ranking para o formato original:\n\nMostrar:\n\nposição;\nnome público do servidor;\ntotal de conteúdos;\nquantidade adicionada nos últimos 7 dias.\n\nNão usar ID interno ou dados mascarados.\n\nNão alterar o ranking durante a sincronização.\n\nA sincronização deve apenas atualizar os dados:\n\nquantidade de conteúdos;\nnovos conteúdos;\nhistórico.\n\nSeparar:\n\nlógica de coleta/sincronização;\nlógica de exibição do ranking.\n\nTambém corrigir os conteúdos que continuam sem capa e com rota /movie/null."
                ) : (
                ...
                "Transformar Radar IPTV em sincronização automática exclusiva do Admin\n\nA sincronização do Radar IPTV deve deixar de depender do usuário ficar acompanhando a tela.\n\nImplementar sincronização automática em background usando Core AWS.\n\nRegras:\n\nSomente ADMIN pode:\niniciar manualmente;\nconfigurar frequência;\nvisualizar logs completos.\nRevendedores:\nnão devem ter acesso ao botão de sincronização;\nnão devem iniciar jobs;\napenas visualizar resultados permitidos.\nCriar rotina automática:\n\nExemplo:\n\nsincronizar a cada 6 horas;\nou uma vez por dia;\nprocessar servidores IPTV válidos automaticamente.\nO processamento deve continuar mesmo:\nfechando navegador;\nsaindo da página;\ndesligando computador.\nCriar painel de acompanhamento:\n\nÚltima sincronização:\nData/hora\n\nStatus:\n✅ concluída\n🔄 processando\n⚠️ erros\n\nMostrar:\n\nservidores processados;\nconteúdos encontrados;\nnovos conteúdos;\nerros encontrados.\nEnviar alerta Telegram somente para o administrador quando:\nsincronização terminar;\nencontrar muitos conteúdos novos;\nocorrer erro crítico.\n\nO Core AWS deve ser o responsável pelo processamento, não o navegador.\n\nEu também adicionaria:\n\nCriar um botão apenas no Admin:\n\"Executar sincronização agora\"\n\nAo clicar, iniciar o job e fechar o modal automaticamente. e O Radar está encontrando conteúdos corretamente, porém os cards continuam sem capa e os detalhes não abrem.\n\nProblema identificado:\n\nA rota está gerando:\n\n/app/inteligencia/movie/null\n\nIsso indica que o conteúdo está sem ID válido.\n\nCorrigir:\n\nGarantir que todo conteúdo salvo tenha:\ntmdb_id;\ntipo (movie/serie);\nposter_path;\nbackdrop_path;\nnome original;\nnota TMDB.\nNa sincronização:\n\nIPTV → normalização do título → busca TMDB → salvar vínculo completo.\n\nCriar uma rotina de reparação para conteúdos já encontrados:\nidentificar registros sem tmdb_id;\nbuscar novamente no TMDB;\npreencher capa e informações;\natualizar sem apagar primeira detecção.\nNo frontend:\nnunca gerar URL /movie/null;\nse faltar ID, desabilitar clique ou mostrar mensagem de conteúdo pendente.\nValidar que o poster_url está sendo retornado pela API."
              )}
            </DialogDescription>
