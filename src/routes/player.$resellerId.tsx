              ...
              <div className="pt-6 border-t border-white/5 space-y-3">
                ...
                <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                  <h4 className="text-blue-400 font-bold mb-2 flex items-center gap-2">
                    <ShieldCheck className="h-4 w-4" /> Plano de ação — Correção definitiva do Web Player Stream Proxy
                  </h4>
                  <p className="text-[10px] text-blue-200/70 leading-relaxed whitespace-pre-wrap">
                    {"A investigação já encontrou a causa raiz. Agora criar e executar um plano de ação técnico para concluir a correção.\n\nProblema confirmado\n\nO Web Player abre normalmente, porém filmes e séries ficam:\n\nTela preta;\nTempo 0:00;\nSem carregamento de mídia.\n\nCausa identificada:\n\nO painel gera a URL do conteúdo corretamente.\nO servidor IPTV bloqueia requisições vindas do IP do painel com Cloudflare 403.\nO fallback para o Core AWS existe, porém a rota atual do Core ainda tenta acessar banco e retorna erro.\nO Core precisa funcionar como worker stateless, recebendo uma URL assinada e entregando os bytes do vídeo.\nObjetivo\n\nFazer o fluxo funcionar:\n\nWeb Player\n↓\nPainel valida sessão\n↓\nPainel gera URL assinada temporária\n↓\nCore AWS recebe requisição\n↓\nCore busca mídia pelo IP AWS\n↓\nCore retorna vídeo para usuário\n↓\nPlayer reproduz\nEtapa 1 — Atualizar Core Stream Proxy\n\nArquivo principal:\n\nsrc/routes/api/public/core/stream.ts\n\nImplementar:\n\nRemover qualquer dependência de Supabase no Core;\nNão consultar banco;\nValidar somente assinatura enviada pelo painel;\nBuscar o stream no servidor IPTV;\nRepassar resposta ao usuário.\n\nObrigatório suportar:\n\nRange Request;\nHTTP 206 Partial Content;\nContent-Type correto;\nContent-Length;\nAccept-Ranges;\nmp4;\nmkv;\nts;\nm3u8.\nEtapa 2 — Ajustar Painel\n\nVerificar:\n\nsrc/routes/player.$resellerId.tsx\nsrc/lib/player.functions.ts\nsrc/lib/player.server.ts\n\nConfirmar:\n\nURL assinada sendo criada corretamente;\nExpiração da assinatura;\nFilme usando container_extension real;\nSéries usando extensão correta do episódio.\nEtapa 3 — Deploy do Core AWS\n\nApós alterar o código:\n\nrebuild da imagem;\nrecriar container;\nconfirmar que core.streammonitor.site está rodando a versão nova.\n\nNão alterar arquitetura.\n\nEtapa 4 — Teste real\n\nTestar:\n\nFilme\n\nExemplo:\n\nO Afinador\nID: 6079122\n\nValidar:\n\nStatus HTTP:\nContent-Type:\nRange:\nBytes entregues:\n\nEsperado:\n\n206 Partial Content\nSérie\n\nTestar:\n\n1 temporada;\n1 episódio.\n\nConfirmar que segue o mesmo fluxo.\n\nEtapa 5 — Logs temporários\n\nAdicionar:\n\nRequest:\n\n[STREAM REQUEST]\ncontent:\ntype:\nserver:\nsignature:\n\nResposta:\n\n[STREAM RESPONSE]\nstatus:\ncontent-type:\nrange:\nlength:\nerror:\n\nNunca registrar senha.\n\nCritério de conclusão\n\nSó considerar concluído quando:\n\n✅ Filme reproduzir no Web Player\n✅ Série reproduzir no Web Player\n✅ Proxy retornar bytes reais\n✅ Não existir 403 do painel\n✅ Core continuar sem banco\n✅ Auditoria registrar sucesso real\n\nNão criar apenas documentação ou alterar textos. Executar as alterações técnicas acima e retornar evidências do teste final."}
                  </p>
                </div>
                <Button
                  variant="outline"
