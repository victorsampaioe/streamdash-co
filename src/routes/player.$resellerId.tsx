          ...
          <div className="p-6 md:p-12 max-w-2xl mx-auto space-y-8">
            ...
            <Card className="p-6 bg-white/5 border-white/10 space-y-6">
              ...
              <div className="pt-6 border-t border-white/5 space-y-3">
                <h2 className="text-lg font-semibold"># PROMPT — Confirmar Deploy do Core Worker e Provar o 206 com Teste Real</h2>
                <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                  <h4 className="text-blue-400 font-bold mb-2 flex items-center gap-2">
                    <ShieldCheck className="h-4 w-4" /> Diagnóstico de Produção e Validação 206
                  </h4>
                  <p className="text-[10px] text-blue-200/70 leading-relaxed whitespace-pre-wrap">
                    {"Já confirmei de fora que `https://core.streammonitor.site/api/public/health?deep=1` está respondendo `\"status\":\"ok\"` e `\"DATABASE\": false` — ou seja, o Core está no ar como worker. Mas isso **não prova** que a versão nova do `src/routes/api/public/core/stream.ts` (com Range Request/206) está realmente publicada, nem que o player reproduz de verdade. Preciso que você feche isso agora com execução real, não com \"deve estar funcionando\".\n\nExecute e retorne evidência de cada item abaixo:\n\n## 1. Confirmar que a versão nova está em produção\n- Confirme, com evidência concreta (hash de commit, timestamp de build, ou log de deploy), que o container do Core em produção está rodando o `stream.ts` reescrito (sem Supabase, validando só assinatura, com suporte a Range).\n- Se ainda não publicou, publique agora e recrie o container.\n\n## 2. Gerar URL assinada real e testar com Range\n\n**Filme** — \"O Afinador\" (ID 6079122):\n- Gere a URL assinada real via fluxo do painel (mesma lógica que o player usa).\n- Rode:\n```bash\ncurl -i -H \"Range: bytes=0-1048575\" \"<URL_ASSINADA_GERADA>\"\n```\n- Cole a resposta completa (status line + todos os headers).\n\n**Série** — 1 episódio qualquer disponível:\n- Mesmo processo: gerar URL assinada real do episódio e rodar o curl com Range.\n- Cole a resposta completa.\n\n## 3. Evidência esperada, em ambos os casos\n\n```\n[STREAM RESPONSE]\nstatus:\ncontent-type:\ncontent-range:\naccept-ranges:\ncontent-length:\n```\n\nResultado esperado: `HTTP/1.1 206 Partial Content`, com `content-range` batendo com o range pedido, `accept-ranges: bytes` presente, e `content-length` correspondente ao tamanho do chunk (não o tamanho zero e não o arquivo inteiro).\n\n## 4. Teste no player de verdade\nDepois do curl confirmar 206, abra o Web Player, entre no filme e no episódio testados, e confirme visualmente que o vídeo começa a reproduzir (não fica em 0:00 / tela preta). Descreva o resultado real (funcionou / não funcionou / erro específico).\n\n## Não aceito como resposta\n- \"A versão nova já deve estar no ar\" sem hash/timestamp de deploy.\n- \"O endpoint deve suportar Range agora\" sem o curl real colado.\n- Qualquer resposta sem os headers reais de `[STREAM RESPONSE]` para filme e série.\n\nSe algo falhar, quero o erro real (status code + corpo da resposta), não um resumo genérico — para eu conseguir apontar exatamente o próximo ajuste."}
                  </p>
                </div>
                <Button
                  variant="outline"
