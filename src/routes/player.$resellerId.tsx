          ...
          <div className="p-6 md:p-12 max-w-2xl mx-auto space-y-8">
            ...
            <Card className="p-6 bg-white/5 border-white/10 space-y-6">
              ...
              <div className="pt-6 border-t border-white/5 space-y-3">
                <h2 className="text-lg font-semibold"># Investigação: falhas específicas do `iptv-player-proxy` na Auditoria Core AWS</h2>
                <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                  ...
                  <p className="text-xs text-yellow-200/80 leading-relaxed whitespace-pre-wrap">
                    {"## Contexto\nO Core AWS já está funcionando e a comunicação automática foi corrigida — confirmado pelos pontos abaixo, que **não precisam ser re-testados**:\n- ✅ `probe-http` automático funcionando\n- ✅ Auditoria registrando sucesso corretamente para `probe-http`\n- ✅ Core respondendo HTTP 200\n\nPorém, especificamente a task **`iptv-player-proxy`** continua falhando na Auditoria Core AWS, mesmo com o restante do fluxo saudável. Como `probe-http` já funciona ponta a ponta, o problema não é genérico da comunicação painel↔Core — é específico dessa task, provavelmente no payload ou nos dados que ela depende (credenciais do servidor IPTV).\n\n**Não alterar a arquitetura.** Mesma regra de antes: painel = dono do banco, Core AWS = worker stateless, comunicação via `CRON_SECRET`. O objetivo é só encontrar e corrigir a causa da falha desta task específica.\n\n---\n\n## Fluxo a investigar\n```\nPainel\n  ↓\nMonitoramento/Player (o que dispara a checagem do proxy)\n  ↓\ncore-api.server.ts\n  ↓\n/api/public/core/task\n  ↓\nCore AWS\n```\n\n## O que preciso que seja verificado\n\n### 1. Payload enviado para `iptv-player-proxy`\n- Capturar exatamente o payload que está sendo montado e enviado para essa task específica (não o de `probe-http`, que já sabemos que funciona).\n- Confirmar quais campos são obrigatórios para essa task no Core (provavelmente `host`, `username`, `password` e `stream`/URL do stream) e se **todos** estão chegando preenchidos — nenhum `undefined`, `null` ou string vazia.\n- Confirmar a origem de cada um desses campos: de onde vêm as credenciais (username/password) usadas — são lidas corretamente da tabela de servidores/sessão, ou existe algum ponto do código que ainda espera esses dados de um lugar errado?\n\n### 2. Comparar teste manual x chamada automática\nFazer um teste manual do `iptv-player-proxy` (curl direto no Core, com um payload montado à mão, com credenciais reais de um servidor que se sabe que funciona) e comparar com o payload que a chamada automática de fato envia. Se o teste manual funcionar e o automático não, a diferença entre os dois payloads é a causa mais provável.\n\n### 3. Resposta do Core para essa task\nQuando a chamada automática falha, capturar a resposta completa do Core (status HTTP, corpo da resposta, mensagem de erro) — não assumir que é timeout genérico. Pode ser:\n- Erro de autenticação no servidor IPTV de origem (credenciais erradas/expiradas).\n- Erro de formato de URL do stream.\n- Timeout real de rede/servidor IPTV de origem instável.\n- Erro de parsing/validação no próprio Core antes de tentar conectar no IPTV.\n\n### 4. Logs temporários\nAdicionar, especificamente na rota dessa task:\n\nNo envio:\n```\n[CORE REQUEST]\ntask: iptv-player-proxy\npayload: { host, username: presente/ausente, password: presente/ausente, stream }\ntimestamp: xxx\n```\n(Nunca logar a senha em texto puro — só indicar presença/ausência do campo.)\n\nNa resposta:\n```\n[CORE RESPONSE]\nstatus: xxx\nresult: xxx\nerror: xxx\n```\n\nDepois, executar uma chamada automática real (deixar o scheduler rodar naturalmente, não só teste manual) e usar esses logs para identificar a causa exata.\n\n### 5. Verificar se a Auditoria está misturando dado antigo com novo\nMesma checagem que já foi feita para `probe-http`: confirmar se as falhas exibidas de `iptv-player-proxy` são execuções recentes (pós-correção) ou se ainda há histórico antigo poluindo a taxa de sucesso exibida no painel.\n\n---\n\n## Entregável esperado\n1. Causa raiz identificada com evidência (log real, não suposição) de por que `iptv-player-proxy` falha enquanto `probe-http` funciona.\n2. Correção aplicada no ponto exato do problema.\n3. Print/log de uma execução **automática** (do scheduler, não teste manual) de `iptv-player-proxy` com sucesso.\n4. Confirmação de que a Auditoria Core AWS passa a refletir corretamente o status real dessa task, sem misturar com histórico anterior à correção."}
                  </p>
                </div>
                <Button
                  variant="outline"
