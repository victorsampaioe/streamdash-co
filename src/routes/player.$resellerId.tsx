          ...
          <div className="p-6 md:p-12 max-w-2xl mx-auto space-y-8">
            ...
            <Card className="p-6 bg-white/5 border-white/10 space-y-6">
              ...
              <div className="pt-6 border-t border-white/5 space-y-3">
                <h2 className="text-lg font-semibold">Investigação: player abre mas fica em tela preta, 0:00 (filmes e séries)</h2>
                <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                  <h4 className="text-yellow-400 font-bold mb-2 flex items-center gap-2">
                    <ShieldCheck className="h-4 w-4" /> Diagnóstico da Cadeia de Reprodução
                  </h4>
                  <p className="text-xs text-yellow-200/80 leading-relaxed whitespace-pre-wrap">
                    {"Contexto e progresso já confirmado\n\nAvanço em relação ao chamado anterior — não precisa re-testar isso:\n\n✅ Player abre\n\n✅ Elemento de vídeo (<video>) aparece na tela\n\n✅ Catálogo e login continuam funcionando normalmente\n\n❌ Reprodução não inicia — tempo permanece travado em 0:00, tela preta\n\nIsso é mais específico que \"não reproduz\": o elemento de vídeo existe, então o problema está entre ele receber a URL do stream e o navegador conseguir decodificar/tocar o conteúdo. Ou seja, provavelmente não é mais um problema de UI/roteamento, e sim de dado (URL ausente/inválida) ou de entrega (proxy/CORS/formato).\n\nNão alterar a arquitetura. O Core worker continua stateless; o painel continua responsável pelo banco e pela geração das URLs.\n\nFluxo a investigar\n\nCliente clica no filme/série\n  ↓\nplayer.functions.ts\n  ↓\nplayer.server.ts\n  ↓\ngeração da URL do stream\n  ↓\ncore stream proxy (/api/public/core/stream)\n  ↓\nentrega do vídeo para o navegador\n\n\nLogs temporários a adicionar\n\n1. Ao gerar o stream:\n\n[STREAM CREATE]\ncontent: xxx\ntype: xxx\nserver_id: xxx\nurl presente: sim/não\nformato: m3u8/mp4/ts\n\n\n2. Ao chamar o proxy:\n\n[STREAM PROXY]\nurl destino: xxx\nstatus HTTP: xxx\ncontent-type: xxx\nrange: xxx\ncontent-length: xxx\n\n\n3. Ao retornar para o navegador:\n\n[STREAM RESPONSE]\nstatus: 200/206\nmime: xxx\nerro: xxx\n\n\n(Nunca logar a URL com usuário/senha embutidos — se a URL contiver credenciais, logar só o host e o path, mascarando a parte de autenticação.)\n\nPontos a verificar, na ordem em que a cadeia acontece\n\nA URL do filme/série está sendo criada corretamente? Conferir se url presente é realmente \"sim\" e se o formato (m3u8/mp4/ts) bate com o que o conteúdo de fato é no servidor Xtream — não assumir, checar o valor real logado.\n\nO proxy está receiving uma URL válida? Confirmar que a URL chega inteira e bem formada até a chamada do proxy (sem truncar, sem caracteres especiais quebrando o parsing).\n\nO servidor IPTV aceita a conexão? Testar a URL gerada diretamente (via curl/Postman, fora do navegador) para isolar se o servidor de origem está aceitando a requisição do proxy ou recusando (401/403), travando (timeout) ou aceitando normalmente.\n\nO navegador recebe o Content-Type correto? Verificar se a resposta chega com application/vnd.apple.mpegurl (ou equivalente) para .m3u8, ou video/mp4 para VOD direto — um Content-Type errado pode fazer o navegador/hls.js simplesmente não reconhecer o conteúdo, mesmo que os bytes estejam corretos.\n\nRange requests estão funcionando (206 Partial Content)? Confirmar se o proxy responde corretamente a requisições com header Range — sem isso, filmes/VOD tendem a não iniciar ou travar em 0:00 mesmo com o resto certo.\n\nO HLS (.m3u8) está sendo reescrito corretamente? Se o proxy reescreve o manifest .m3u8 para apontar os segmentos de volta para o próprio proxy (necessário para contornar CORS/credenciais), confirmar que essa reescrita está gerando URLs válidas e acessíveis — um manifest mal reescrito é uma causa muito comum de \"elemento de vídeo existe mas nunca sai do 0:00\".\n\nClassificar a causa em uma das 4 categorias\n\nAo final da investigação, preciso que a resposta identifique claramente em qual ponto está o problema:\n\nA) Geração da URL — a URL nunca chega correta/completa até o proxy.\n\nB) Proxy do stream — a URL chega certa, mas o proxy não entrega o conteúdo corretamente pro navegador (status, headers, Content-Type, range).\n\nC) Bloqueio do servidor IPTV — o proxy repassa certo, mas o servidor de origem recusa/bloqueia a conexão.\n\nD) Player recebendo formato incompatível — os bytes chegam, mas o hls.js/elemento de vídeo não consegue decodificar (manifest mal formado, MIME errado, codec não suportado).\n\nDiferenciação por tipo de conteúdo\n\nTestar separadamente filme e série/episódio — se um funcionar e o outro não, isso já aponta que a causa é específica da lógica daquele tipo (ex: forma diferente de montar a URL para série vs. filme), não do proxy genérico.\n\nEntregável esperado\n\nLog real (não suposição) mostrando em qual das 4 categorias (A/B/C/D) está a causa, para filme e para série separadamente.\n\nCorreção aplicada nel ponto identificado.\n\nConfirmação de reprodução funcionando de verdade — o vídeo rodando na tela, com print ou gravação mostrando o tempo avançando (não parado em 0:00)."}
                  </p>
                </div>
                <Button
                  variant="outline"
