          ...
          <div className="p-6 md:p-12 max-w-2xl mx-auto space-y-8">
            ...
            <Card className="p-6 bg-white/5 border-white/10 space-y-6">
              ...
              <div className="pt-6 border-t border-white/5 space-y-3">
                <h2 className="text-lg font-semibold"># Investigação: Web Player não reproduz filmes e séries (cadeia de reprodução)</h2>
                <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                  <h4 className="text-yellow-400 font-bold mb-2 flex items-center gap-2">
                    <ShieldCheck className="h-4 w-4" /> Diagnóstico do Web Player
                  </h4>
                  <p className="text-xs text-yellow-200/80 leading-relaxed whitespace-pre-wrap">
                    {"## Contexto\nA correção da arquitetura do Core já resolveu a Auditoria — `probe-http` e `iptv-player-proxy` funcionando corretamente do lado do monitoramento. **Este chamado é diferente: não é sobre Auditoria/monitoramento, é sobre a experiência real do cliente final tentando assistir.** Mesmo com o Core saudável, o Web Player ainda não reproduz filmes nem séries.\n\n**Não mexer na arquitetura do Core novamente.** O problema agora está na cadeia de reprodução do Web Player em si — entre o clique do cliente e o vídeo aparecer na tela.\n\n---\n\n## Fluxo esperado (mapear onde exatamente quebra)\n```\nCliente abre Web Player\n  ↓\nLogin Xtream\n  ↓\nBusca catálogo\n  ↓\nCliente escolhe filme/série\n  ↓\nPainel consulta credenciais no banco\n  ↓\nGera URL do stream\n  ↓\nProxy entrega mídia\n  ↓\nPlayer reproduz\n```\n\nPreciso saber, com evidência (log real), em **qual desses elos exatos** a cadeia para de funcionar — não uma suposição genérica de \"problema no proxy\".\n\n---\n\n## Logs temporários a adicionar\n\n**1. Ao clicar no filme/série:**\n```\n[PLAYER REQUEST]\ncontent_id: xxx\ntype: movie/series/episode\nserver_id: xxx\n```\n\n**2. Ao gerar a URL do stream:**\n```\n[STREAM URL]\nhost: xxx\ntipo: xxx\nurl gerada: presente/ausente\n```\n(Nunca gravar senha ou a URL completa com credenciais embutidas em log — só confirmar presença/ausência e o host/tipo.)\n\n**3. Ao chamar o proxy:**\n```\n[PLAYER PROXY]\nstatus: xxx\ncontent-type: xxx\nrange: xxx\nerro: xxx\n```\n\n---\n\n## Pontos específicos a investigar\n\n1. **O Web Player está recebendo a URL do filme/série?**\n   Confirmar se `content_id` chega corretamente do clique até a Server Function que busca as credenciais, e se a URL do stream é de fato gerada (não fica `undefined`/vazia em algum ponto).\n\n2. **O endpoint de stream está retornando 200/206?**\n   Checar o status HTTP real da resposta do proxy quando o player tenta carregar o vídeo. Um 206 (Partial Content) é esperado para permitir avançar/retroceder — se estiver sempre voltando 200 completo ou erro (403/404/500), anotar exatamente qual.\n\n3. **O proxy está conseguindo acessar o servidor IPTV de origem?**\n   Testar isoladamente se a chamada do proxy ao servidor IPTV (com as credenciais reais) está funcionando — separar \"proxy não consegue falar com o Xtream de origem\" de \"proxy fala com o Xtream, mas não entrega certo pro navegador\".\n\n4. **O navegador está recebendo vídeo ou erro do HLS.js?**\n   Verificar no console do navegador (DevTools) se o `hls.js` está reportando algum erro específico (ex: `manifestLoadError`, `levelLoadError`, `bufferStalledError`) — isso ajuda a identificar se o problema é no manifest `.m3u8`, no segmento de vídeo em si, ou em CORS/headers.\n\n## Diferenciação importante\nFazer o teste separadamente para:\n- **Filme (VOD)** — geralmente mais simples, formato de arquivo único.\n- **Série/episódio** — pode ter uma lógica de busca de URL diferente do filme.\n- **Canal ao vivo**, se ainda não tiver sido validado — formato `.ts`/`.m3u8`, que é o mais sensível a esse tipo de problema.\n\n---\n\n## Entregável esperado\n1. Identificação exata do elo da cadeia onde a reprodução quebra (com log/print real, não suposição), separado por tipo de conteúdo (filme, série, ao vivo).\n2. Causa raiz de cada falha encontrada.\n3. Correção aplicada.\n4. Confirmação de reprodução funcionando de fato — vídeo rodando na tela, print/gravação de tela mostrando o filme/série tocando, não só ausência de erro no console."}
                  </p>
                </div>
                <Button
                  variant="outline"
