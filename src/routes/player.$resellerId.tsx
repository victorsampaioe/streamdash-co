          ...
          <div className="p-6 md:p-12 max-w-2xl mx-auto space-y-8">
            ...
            <Card className="p-6 bg-white/5 border-white/10 space-y-6">
              ...
              <div className="pt-6 border-t border-white/5 space-y-3">
                <h2 className="text-lg font-semibold">Execução obrigatória: diagnóstico real de reprodução (não é mais para planejar)</h2>
                <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                  <h4 className="text-yellow-400 font-bold mb-2 flex items-center gap-2">
                    <ShieldCheck className="h-4 w-4" /> Investigação em Ambiente Real
                  </h4>
                  <p className="text-xs text-yellow-200/80 leading-relaxed whitespace-pre-wrap">
                    {"## Contexto\nJá foram entregues roteiros de investigação para esse mesmo problema (tela preta, 0:00 em filmes/séries) e a resposta até agora ficou no nível de \"vou verificar X, Y, Z\". **Preciso que agora seja executado de fato, em ambiente real, com um teste real de reprodução — e que o resultado venha com dado concreto, não com plano.**\n\nNão alterar a arquitetura do Core. O Core worker já está funcionando — o problema está entre a geração da URL e a entrega do vídeo no navegador.\n\n---\n\n## O que precisa ser feito, na ordem\n\n### 1. Rodar um teste real de reprodução agora\nAbrir o player, escolher **1 filme** e **1 episódio de série** reais, e reproduzir de fato — não simular, não analisar código estático sem rodar.\n\n### 2. Capturar e me entregar, para cada um dos dois testes:\n\n**a) URL de mídia gerada**\n- A URL existe? (sim/não)\n- Qual formato está sendo usado (`mp4`, `ts`, `m3u8`)?\n- A URL é válida (aponta para um recurso real, sem campos faltando)?\n\n**b) Resposta do proxy** — registrar exatamente:\n```\n[STREAM PROXY]\nStatus HTTP: xxx\nContent-Type: xxx\nRange: xxx\nContent-Length: xxx\nErro: xxx\n```\nConfirmar explicitamente:\n- Está retornando **HTTP 206 Partial Content** para VOD (filme/episódio)?\n- Está retornando um **manifest HLS válido** para `.m3u8`?\n- Os **segmentos** do HLS estão carregando (não só o manifest)?\n\n**c) Console do navegador (DevTools) durante a tentativa de reprodução**\nConfirmar o que o player efetivamente recebe:\n- Bytes de vídeo chegando?\n- Erro 403?\n- Erro 404?\n- Erro de CORS?\n- Erro de formato incompatível (ex: erro do hls.js tipo `manifestLoadError`, `bufferStalledError`)?\n\n### 3. Testes obrigatórios (os dois, não apenas um)\n- 1 filme\n- 1 episódio de série\n\nSe o comportamento for diferente entre os dois, reportar a diferença — isso já indica se a causa é genérica (proxy) ou específica de um tipo de conteúdo.\n\n---\n\n## Entregável esperado — formato objetivo\n\nPreciso de uma frase de causa raiz no estilo de uma destas (ou a que realmente foi encontrada, com o dado real por trás):\n- \"A URL estava vazia porque `campo X` não era passado de `arquivo Y` para `arquivo Z`.\"\n- \"O proxy retornava HTTP 200 mas sem bytes no corpo — problema em `arquivo/linha X`.\"\n- \"O header `Range` era ignorado pelo proxy, então o navegador nunca conseguia iniciar a decodificação.\"\n- \"O servidor IPTV de origem recusava a conexão do proxy (401/403) — checar credenciais/IP.\"\n- \"O manifest `.m3u8` estava sendo reescrito incorretamente pelo proxy, gerando URLs de segmento inválidas.\"\n\nJunto disso, preciso de:\n1. **Causa raiz encontrada** (com o log/evidência real que comprova, não suposição).\n2. **Arquivo(s) responsável(is)** pela causa.\n3. **Correção aplicada.**\n4. **Teste final comprovando reprodução funcionando de verdade** — vídeo rodando na tela, tempo avançando (não 0:00), print ou gravação de tela, para filme e para série separadamente.\n\nNão aceito como entrega \"revisei o código e ajustei X\" sem o log/print mostrando o problema real e o resultado funcionando depois. O objetivo desta rodada é sair de \"planejamento\" para \"prova concreta\"."}
                  </p>
                </div>
                <Button
                  variant="outline"
